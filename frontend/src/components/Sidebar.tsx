        ...
        >
            ...
            <div className="p-6 flex items-center justify-between">
                {!collapsed && (
                    <div className="flex items-center space-x-3">
                        ...
                        <span className="font-bold text-slate-800 tracking-tight">EbookLib</span>
                    </div>
                )}
                <button
            ...
            <div className="flex-1 overflow-y-auto px-4 space-y-8 scrollbar-hide py-4">
                ...
                <div className="space-y-1">
                    {!collapsed && <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-3 mb-2">Main</p>}
                    {menuItems.map((item) => (
                        <button
                            key={item.id}
                ...
                <div className="space-y-1">
                    <div className="flex items-center justify-between px-3 mb-2">
                        {!collapsed && <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Collections</p>}
                        {!collapsed && (
                            <button
                    ...
                    {showNewCollectionInput && !collapsed && (
                        <div className="px-3 mb-2 animate-in slide-in-from-top-1 duration-200">
                            <input
                                autoFocus
                                className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-xs outline-none focus:ring-1 focus:ring-blue-500"
                    ...
                    {collections?.map((col: any) => (
                        ...
                        >
                            <div className="flex items-center space-x-3 flex-1 min-w-0" onClick={() => onFilterChange(`collection:${col.id}`)}>
                                {!collapsed && <div className="w-1.5 h-1.5 rounded-full bg-slate-300" />}
                                {!collapsed ? (
                                    editingCollectionId === col.id ? (
                                        <input
                                            autoFocus
                                            className="w-full bg-white border border-blue-300 rounded px-1 outline-none font-normal"
                            ...
                            {!collapsed && editingCollectionId !== col.id && (
                                <div className="hidden group-hover:flex items-center space-x-1">
                                    <button
                                        ...
                                        className="p-1 hover:bg-white rounded text-slate-400 hover:text-blue-500 transition-colors"
                                    >
                ...
                {!collapsed && (
                    <div className="space-y-1">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-3 mb-2">Tags</p>
                ...
                {!collapsed ? (
                    <div className="flex items-center space-x-3 p-2">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-slate-500 uppercase font-medium">Librarian</p>
                        </div>
                    </div>
                ) : (
